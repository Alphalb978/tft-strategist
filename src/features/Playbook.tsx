...
export function Playbook({
  ...
  return (
    <div className={activeMode ? 'playbook-page match-mode' : 'playbook-page'}>
        ...
        <div>
          ...
          <p>
            ...
            <span className="inline-badge">
              {plan.id.startsWith('external-') ? 'External Reference' : plan.evidence} ·{' '}
      ...
      <nav className="companion-tabs" aria-label="Comp view">
        <button aria-pressed={view === 'quick'} onClick={() => setView('quick')}>
          Quick
        </button>
        <button aria-pressed={view === 'details'} onClick={() => setView('details')}>
          Details · stages / items / stats
        </button>
      </nav>
      {view === 'quick' && snapshotContext && session?.compatibility.state === 'stale' && (
        <p className="panel">
          Historical session · open Details for the preserved decision. Current scoring is
      ...
      {view === 'details' && (
        <>
          {activeMode && (
            <div className="match-now">
              <div>
                ...
                <strong>
                  {stage?.instruction.value ??
                    stage?.instruction.note ??
                    'Review the target roster'}
                </strong>
                <br />
          ...
          <section className="panel stage-progression" id="stages">
            ...
            <div
              className="strategy-stage-tabs"
              role="tablist"
              aria-label="Strategy stage progression"
            >
            ...
            <section className="panel decision-panel" id="decision-map">
              ...
              {decisionNode ? (
                    ...
                    <div className="decision-options">
                      {outgoing.map((edge) => (
                        <button key={edge.id} onClick={() => selectDecisionEdge(edge.id)}>
                          ...
                          <strong>
                            {
                              plan.strategy.decisionMap.nodes.find((node) => node.id === edge.to)
                                ?.label
                            }
                          </strong>
                          <ArrowRight size={15} />
                ...
                <Unavailable
                  ...
                  note={plan.strategy.decisionMap.note}
                />
              )}
            </section>
          </div>
          ...
          <details className="planner-verification">
            <summary>
              Team Planner ·{' '}
              {plannerCandidate.ok
                ? 'Client verified · roster only'
                : 'Unavailable for this roster'}
            </summary>
